...
// 那么处理以后变成01111111100
// 这时候我们发现左侧和右侧还有0
// 那我们要保留多的一边, 再把少的一边完全删除
// 即1111111100, 最终答案就是11111111的值36 + 00的值3, 为39
// 这里要注意的是, 如果0和1的数量相同, 我们就要看两侧的0和1谁比较多
// 这样很麻烦, 所以在程序实现中我们干脆把留下0和1的情况都计算, 直接取最大值
#include<iostream>
#include<vector>
#include<string>
...
void solve() {
    ...
    // 求出保留0或者保留1这两个情况中的最大值即为答案
    int ans = max(get_ans('0', cnt), get_ans('1', cnt));
    cout << ans << endl;
}
