...
public class Main{
    public static void main (String[] args) {
        Scanner sc = new Scanner(System.in);
        Main exe = new Main();	//用于调用类内方法 findDepth
        while(sc.hasNextLine()){
            int count = Integer.parseInt(sc.nextLine());
            //parent[i]：编号为 i 的人的父亲是 parent[i]。例如 3 的父亲是 5，则有 parent[3]=5
            int[] parent = new int[21];		//编号取值范围: [1,20]
            while(count-- > 0){
            ...
            int depth_XM = exe.findDepth(parent,1),depth_XY = exe.findDepth(parent,2);
    ...
    */
    public int findDepth(int[] parent,int son){
        int count = 0;
