...
public class Main {
    ...
    public static int maxPathSum(int[][] grid) {
        ...
        *所以当 i==0 或 j == 0 时(i-1) 或 (j - 1)产生数组越界问题，故而需要先对dp数组的第一行以及第一列初始化
        *可实际上，我们可以建立一个(m+1)*(n+1)的dp数组，并将地推公式更改为：
        * dp[i][j]=Math.max(dp[i-1][j],dp[i][j-1]) + grid[i-1][j-1] <====注意这里的[i-1][j-1]
        *由于数组元素的初始值为 0，所以经过修改后的递推公式依旧对于首行或首列依旧可以得出与上述“初始化”代码相同的结果
        *============【综上】可以考虑以下“简化”版本解法:============
